...
function ElectionSelectApp() {
    const navigate = useNavigate();
    const location = useLocation();
  const isAdmin = location.state.isAdmin;
    const handleCardClick = (action) => {
        switch(action) {
            case 'nationalElection':
              navigate('/candidatesProfile', { state: { isAdmin } });
              break;
            case 'localElections':
              navigate('/localElection', { state: { isAdmin } });
              break;
            default:
              break;
          }
          ...
          <Row>
            <Col>
              <Card className="mt-4 mb-4" style={{ 
                ...
                margin: '0 auto',
                padding: '30px' 
              }} onClick={() => handleCardClick('nationalElection')}>
                <Card.Body className='text-center' style={{ fontWeight: 'bold', marginBottom: '20px' }}>
                  <Card.Title style={{fontSize: '2em'}}>NATIONAL ELECTION</Card.Title>
                </Card.Body>
              </Card>
              <Card className="mt-4 mb-4" style={{ 
                backgroundColor: 'rgba(35, 16, 153, 1)', 
                ...
                margin: '0 auto',
                padding: '30px' 
              }} onClick={() => handleCardClick('localElections')}>
                <Card.Body className='text-center' style={{ fontWeight: 'bold', marginBottom: '20px' }}>
                  <Card.Title style={{fontSize: '2em'}}>LOCAL ELECTION</Card.Title>
                </Card.Body>
              </Card>
            </Col>
          </Row>
...
export default ElectionSelectApp;
